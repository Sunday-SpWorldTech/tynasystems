...
app.use(cors({
  origin(origin, callback) {
    ...
    const isVercelDeployment = /^https:\/\/[a-z0-9-]+\.vercel\.app$/i.test(normalized);
    if (allowedOrigins.has(normalized) || isVercelDeployment) return callback(null, true);
...
// Deployment routing probe: accepts POST as well as GET and does not require MongoDB.
// This makes it possible to verify that Vercel is forwarding non-GET API requests
// to the Express function instead of treating /api/* as static content.
app.all('/api/route-check', (req, res) => {
...
// Translation does not depend on MongoDB. Keep it available even if the
// database is temporarily reconnecting, so language selection cannot break login UI.
...
// still starts a normal HTTP server with `npm run dev` or `npm start`.
