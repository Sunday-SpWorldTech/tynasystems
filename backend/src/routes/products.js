...
function normalizeProductPrice(product) {
  ...
  if (slug === 'backend-os-demo' || name.includes('backend os demo')) {
    data.priceUSD = 6.99;
    data.priceNGN = 11184;
  }
  if (slug === 'done-for-you-build' || name.includes('done-for-you backend os build')) {
    data.priceUSD = 599;
    data.priceNGN = 958400;
  }
  if (slug === 'backend-os-template' || name.includes('backend os full template')) {
    data.priceUSD = 149;
    data.priceNGN = 238400;
  }
  return data;
}

export { normalizeProductPrice };
...
router.get('/', async (req, res, next) => {
  try {
    ...
    res.json({ products: (products.length ? products : fallbackProducts).map(normalizeProductPrice) });
  } catch (err) { next(err); }
});
...
router.get('/dashboard/my-products', requireAuth, async (req, res, next) => {
  try {
    ...
    res.json({ orders: orders.map(publicOrder), products: products.map(normalizeProductPrice) });
  } catch (err) { next(err); }
});
...
router.get('/:slug', async (req, res, next) => {
  try {
    ...
    res.json({ product: normalizeProductPrice(product || fallback) });
  } catch (err) { next(err); }
});
