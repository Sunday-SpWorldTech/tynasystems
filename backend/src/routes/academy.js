...
router.post('/verify/school-email', async (req, res, next) => {
  try {
    ...
    if (!emailOtpEnabled()) return res.status(503).json({ message: 'School email OTP is not ready because SMTP email is not configured. Add SMTP_HOST, SMTP_USER and SMTP_PASS in Vercel, or use ID/NIN verification for admin review.' });
    ...
    if (sent?.error || sent?.skipped) return res.status(503).json({ message: 'OTP could not be sent. Check SMTP email settings in Vercel, or submit ID/NIN for admin review.' });
