...
const API_FALLBACK_URL = window.location.origin;
...
function configureGoogleSignin(){
  ...
  if(hasRealGoogleClientId(clientId)){
    ...
    if(msg) showMsg(msg, 'Google login is not configured yet. Add VITE_GOOGLE_CLIENT_ID on the frontend and GOOGLE_CLIENT_ID on the backend in Vercel Environment Variables.', 'info');
  }
}
...
async function api(path, options={}){
  ...
  const request = async (base) => {
    ...
    catch { throw new Error('Unable to reach the Tyna Systems server. Check your internet connection or deployment status.'); }
    const raw = await res.text().catch(()=>'');
    let data = {};
    try { data = raw ? JSON.parse(raw) : {}; } catch { data = {}; }
    if(!res.ok){
      const fallback = res.status === 404
        ? `The server API route is unavailable (${path}).`
        : res.status === 405
          ? 'The API POST request reached the wrong Vercel route. Redeploy the repository-root build with the included vercel.json.'
          : `Server request failed (HTTP ${res.status}). Please try again.`;
  ...
  try { return await request(API_URL); }
    ...
    if(shouldRetryWithLiveBackend(API_URL)){
      try { return await request(API_FALLBACK_URL); } catch(liveErr) { err = liveErr; }
    }
    if(err?.status === 401){
      clearSession();
      const target = location.pathname.split('/').pop() + location.search + location.hash;
      if(!isAuthPageTarget(location.pathname)) location.replace(`login.html?session=expired&redirect=${encodeURIComponent(target)}`);
      throw new Error('Your login session expired. Please sign in again to continue.');
    }
...
async function startPayment(productSlug='backend-os-demo'){
  ...
  const data = await api('/api/payments/initialize',{method:'POST',body:JSON.stringify({email:u.email,name:u.name,productSlug,currency:localStorage.getItem('appCurrency')||'USD'})});
  if(data.authorization_url) location.href = data.authorization_url;
  else throw new Error('Payment could not start. Please try again or use another supported bank card.');
}
function productCard(p, owned=false){
  ...
  const action = owned
    ? ((p.downloadUrl || p.fileUrl) ? `<a class="btn dark small" href="${safe(p.downloadUrl || p.fileUrl)}" target="_blank" rel="noreferrer" data-download="${safe(p.slug)}">Download</a>` : `<span class="badge success">Purchased</span>`)
    : `<button class="btn gold small" data-wallet-buy="${safe(p.slug)}">Pay with Wallet</button><button class="btn dark small" data-buy="${safe(p.slug)}">Pay with Paystack</button>`;
  return `<article class="card product-card">${image}<div><span class="badge info">${safe(p.category || p.deliveryType || 'Product')}</span><h3>${safe(p.name)}</h3><p>${safe(p.subtitle || p.description || '')}</p></div><div class="module row"><b data-price-usd="${Number(p.priceUSD ?? (p.priceNGN ? Number(p.priceNGN) / USD_NGN_RATE : 0))}">${productPrice(p)}</b><div class="app-actions">${action}${p.youtubeUrl?`<a class="btn light small" href="${safe(p.youtubeUrl)}" target="_blank" rel="noreferrer">Tour</a>`:''}</div></div></article>`;
}

async function purchaseWithWallet(productSlug=''){
  if(!productSlug) throw new Error('Product is required.');
  const data = await api('/api/payments/wallet/purchase',{method:'POST',body:JSON.stringify({productSlug})});
  return data;
}
...
async function renderUserWallet(main, u){
  main.innerHTML = `<div class="app-title"><div><span class="eyebrow">User Wallet</span><h1>Wallet & Payments</h1><p>Deposit with Paystack, buy products with wallet balance, or pay Tyna Systems admin directly from your wallet.</p></div><a class="btn light" href="dashboard.html">Back to Workspace</a></div><div class="kpi-grid"><div class="kpi"><span class="muted">Wallet Balance</span><strong id="user-wallet-balance">Loading...</strong></div><div class="kpi"><span class="muted">Currency</span><strong id="user-wallet-currency">...</strong></div><div class="kpi"><span class="muted">Account</span><strong>${safe(u.status || 'active')}</strong></div></div><section class="workspace-grid"><div class="dash-card"><h2>Deposit with Paystack</h2><form class="app-form" data-user-deposit-form><label>Amount<input type="number" name="amount" min="100" step="0.01" required placeholder="Enter amount"></label><div class="notice info">Your authenticated account email (${safe(u.email)}) will be used for secure Paystack checkout. The configured platform allocation is deducted before the remaining amount is credited to your wallet.</div><button class="btn gold" type="submit">Deposit to Wallet</button><div class="hidden" data-form-message></div></form></div><div class="dash-card"><h2>Pay Admin with Wallet</h2><form class="app-form" data-admin-wallet-payment><label>Amount<input type="number" name="amount" min="1" step="0.01" required placeholder="Enter amount"></label><label>Payment purpose<input name="purpose" maxlength="240" required placeholder="Example: Service balance, setup fee, invoice payment"></label><div class="notice info">This transfers value from your wallet balance to the Tyna Systems company/admin wallet and creates a transaction reference.</div><button class="btn dark" type="submit">Pay Admin from Wallet</button><div class="hidden" data-form-message></div></form></div></section><section class="section" style="padding:22px 0 0"><div class="dash-card"><h2>Wallet Transactions</h2><div id="user-wallet-transactions" class="module-list">Loading...</div></div></section>`;

  async function loadWallet(){
    const data = await api('/api/payments/wallet');
    const wallet = data.wallet || {};
    document.getElementById('user-wallet-balance').textContent = moneyCurrency(wallet.balance || 0, wallet.currency || 'NGN');
    document.getElementById('user-wallet-currency').textContent = safe(wallet.currency || 'NGN');
    const list = document.getElementById('user-wallet-transactions');
    const txs = wallet.transactions || [];
    list.innerHTML = txs.length ? txs.map(tx=>{
      const isDebit = tx.type === 'debit';
      const prefix = isDebit ? '-' : '+';
      return `<div class="module"><div class="module row"><div><b>${safe(tx.description || 'Wallet transaction')}</b><small class="muted">${new Date(tx.createdAt).toLocaleString()}${tx.reference?` · ${safe(tx.reference)}`:''}</small></div><div style="text-align:right"><b>${prefix}${moneyCurrency(tx.amount, tx.currency || wallet.currency || 'NGN')}</b><br><span class="badge ${tx.status==='successful'?'success':tx.status==='failed'?'warn':'info'}">${safe(tx.status)}</span></div></div></div>`;
    }).join('') : '<div class="empty-state">No wallet transactions yet.</div>';
  }

  const qs = new URLSearchParams(location.search);
  const reference = qs.get('reference') || qs.get('trxref');
  if(reference && (reference.startsWith('tyna_wallet_') || qs.get('wallet_deposit') === '1')){
    const box = main.querySelector('[data-user-deposit-form] [data-form-message]');
    try{
      const data = await api(`/api/payments/wallet/deposit/verify/${encodeURIComponent(reference)}`);
      showMsg(box, data.message || 'Deposit verified and added to your wallet.', 'success');
      window.history.replaceState({}, document.title, 'dashboard.html#wallet');
    }catch(err){ showMsg(box, err.message, 'error'); }
  }

  main.querySelector('[data-user-deposit-form]').addEventListener('submit', async e=>{
    e.preventDefault();
    const form=e.currentTarget, box=form.querySelector('[data-form-message]');
    try{
      const data=await api('/api/payments/wallet/deposit/initialize',{method:'POST',body:JSON.stringify({amount:form.amount.value})});
      showMsg(box,'Opening secure Paystack checkout...','success');
      window.location.href=data.authorization_url;
    }catch(err){ showMsg(box,err.message,'error'); }
  });

  main.querySelector('[data-admin-wallet-payment]').addEventListener('submit', async e=>{
    e.preventDefault();
    const form=e.currentTarget, box=form.querySelector('[data-form-message]');
    const payload=Object.fromEntries(new FormData(form).entries());
    try{
      if(!confirm(`Pay ${payload.amount} from your wallet to Tyna Systems admin for: ${payload.purpose}?`)) return;
      const data=await api('/api/payments/wallet/pay-admin',{method:'POST',body:JSON.stringify(payload)});
      showMsg(box,`${data.message} Reference: ${data.reference}`,'success');
      form.reset();
      await loadWallet();
    }catch(err){ showMsg(box,err.message,'error'); }
  });

  try{ await loadWallet(); }catch(err){ document.getElementById('user-wallet-transactions').innerHTML=`<div class="notice error">${safe(err.message)}</div>`; }
}
...
async function initDashboard(){
  ...
  const initRef = initQuery.get('reference') || initQuery.get('trxref');
  if(initRef && initRef.startsWith('tyna_wallet_') && location.hash !== '#wallet') location.hash = '#wallet';
  const activeDash = location.hash === '#products' ? 'products' : (location.hash === '#wallet' ? 'wallet' : (location.hash === '#chat' ? 'chat' : (location.hash === '#academy' ? 'academy' : 'dashboard')));
  ...
  if(location.hash === '#wallet') return renderUserWallet(main, u);
  ...
  try{
    ...
    main.querySelectorAll('[data-wallet-buy]').forEach(b=>b.addEventListener('click',async()=>{
      const slug=b.dataset.walletBuy;
      if(!confirm('Pay for this product from your Tyna Systems wallet balance?')) return;
      const original=b.textContent;
      try{
        b.disabled=true; b.textContent='Processing...';
        const data=await purchaseWithWallet(slug);
        alert(data.message || 'Wallet purchase successful.');
...
async function renderStaffWallet(main){
  main.innerHTML = `<div class="app-title"><div><span class="eyebrow">Tyna Systems LLC</span><h1>Wallet & Payout</h1><p>View clean business sales balance and withdraw available company funds.</p></div><a class="btn light" href="#staff-orders">View Orders</a></div><div class="kpi-grid"><div class="kpi"><span class="muted">Gross Sales</span><strong id="wallet-gross">...</strong></div><div class="kpi"><span class="muted">Processing Fees</span><strong id="wallet-fees">...</strong></div><div class="kpi"><span class="muted">Pending Withdrawals</span><strong id="wallet-pending">...</strong></div><div class="kpi"><span class="muted">Available</span><strong id="wallet-available">...</strong></div></div><section class="workspace-grid"><div class="dash-card"><h2>Quick Withdraw</h2><form class="app-form" data-wallet-withdraw><label>Amount (USD)<input type="number" step="0.01" min="1" name="amount" required></label><div class="notice info">Admin withdrawals are locked to the Tyna Systems company bank configured in Vercel Environment Variables.</div><label>Notes<textarea name="notes" placeholder="Example: Company wallet payout"></textarea></label><button class="btn gold" type="submit">Withdraw from Wallet</button><div class="hidden" data-form-message></div></form></div><div class="dash-card"><h2>Wallet Rules</h2><div class="module-list"><div class="module"><b>Sales credit</b><p>Paid orders credit Tyna Systems LLC after Paystack verification.</p></div><div class="module"><b>Charges</b><p>Only standard payment processor fees are considered in available balance when Paystack returns fee data.</p></div><div class="module"><b>Withdrawals</b><p>Pending and completed withdrawals reduce available wallet balance for safer company payout tracking.</p></div></div></div></section><section class="section" style="padding-bottom:0"><div class="dash-card"><h2>Recent Wallet Credits & User Payments</h2><div id="company-wallet-transactions" class="module-list">Loading...</div></div></section><section class="section" style="padding-bottom:0"><div class="dash-card"><h2>Recent Withdrawals</h2><div id="wallet-withdrawals" class="module-list">Loading...</div></div></section>`;
  async function loadWallet(){
    try{ const {wallet}=await api('/api/staff/wallet'); document.getElementById('wallet-gross').textContent=money(wallet.grossUSD); document.getElementById('wallet-fees').textContent=money(wallet.feeUSD); document.getElementById('wallet-pending').textContent=money(wallet.pendingWithdrawalsUSD); document.getElementById('wallet-available').textContent=money(wallet.availableUSD); const amount=main.querySelector('[name=amount]'); if(amount && !amount.value) amount.value=Math.max(0, Number(wallet.availableUSD||0)).toFixed(2); const txBox=document.getElementById('company-wallet-transactions'); if(txBox) txBox.innerHTML=(wallet.transactions||[]).map(tx=>`<div class="module"><div class="module row"><div><b>${safe(tx.description||'Company wallet credit')}</b><p class="muted">${safe(tx.metadata?.userEmail||tx.metadata?.source||'Platform payment')}</p></div><div style="text-align:right"><b>${moneyCurrency(tx.amount, tx.currency||wallet.currency||'USD')}</b><br><small>${safe(tx.reference||'')}</small></div></div></div>`).join('') || '<div class="empty-state">No company wallet credits yet.</div>'; document.getElementById('wallet-withdrawals').innerHTML=(wallet.withdrawals||[]).map(w=>`<div class="module"><div class="module row"><b>${money(w.amount)}</b><span class="badge ${w.status==='completed'?'success':w.status==='rejected'?'warn':'info'}">${safe(w.status)}</span></div><small class="muted">Secured company payout · ${new Date(w.createdAt).toLocaleDateString()}</small></div>`).join('') || '<div class="empty-state">No withdrawals yet.</div>'; }
...
async function renderStaffWithdrawals(main){
  main.innerHTML = `<div class="app-title"><div><span class="eyebrow">Financial System</span><h1>Withdrawals</h1><p>Track company wallet payout requests.</p></div><a class="btn gold" href="#staff-wallet">Withdraw from Wallet</a></div><section class="workspace-grid"><div class="dash-card"><h2>Request Payout</h2><form class="app-form" data-withdrawal-form><label>Amount (USD)<input type="number" name="amount" required></label><div class="notice info">Admin withdrawals are locked to the Tyna Systems company bank configured in Vercel Environment Variables.</div><label>Notes<textarea name="notes" placeholder="Optional note"></textarea></label><button class="btn gold" type="submit">Submit Request</button><div class="hidden" data-form-message></div></form></div><div class="dash-card"><h2>Withdrawal History</h2><div id="withdrawal-list" class="module-list">Loading...</div></div></section>`;
...
async function verifyLoginServiceReady(){
  try{
    const health=await api('/api/health');
    if(health?.env?.mongodb === false) throw new Error('Login service is not connected to the database. Add MONGODB_URI in Vercel Environment Variables and redeploy.');
    if(health?.login?.jwtConfigured === false) throw new Error('Secure login is not configured. Add JWT_SECRET in Vercel Environment Variables and redeploy.');
    return health;
  }catch(err){
    throw new Error(err.message || 'Login service is unavailable.');
  }
}
