...
async function startProjectPayment(payload){
  payload.currency = localStorage.getItem('appCurrency') || 'USD';
  ...
  else throw new Error('Project payment could not start. Your selected display currency will be converted to the available checkout currency.');
}

async function startPayment(productSlug='backend-os-demo'){ if(!isLoggedIn()){ location.href=authUrl('dashboard.html#products'); return; } const data=await api('/api/payments/initialize',{method:'POST',body:JSON.stringify({productSlug,currency:localStorage.getItem('appCurrency')||'USD'})}); if(data.authorization_url) location.href=data.authorization_url; else throw new Error('Payment could not start. Please try again or use another supported bank card.'); }
function initPayments(){
  ...
  document.querySelectorAll('[data-project-payment-form]').forEach(form=>{
    ...
    form.addEventListener('submit',async e=>{ e.preventDefault(); const payload=Object.fromEntries(new FormData(form).entries()); payload.amountUSD=Number(payload.amountUSD||0); try{ showMsg(msg,'Starting secure checkout...','success'); await startProjectPayment(payload); }catch(err){ showMsg(msg,err.message,'error'); } });
  });
