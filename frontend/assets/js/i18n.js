...
const i18n = {
  ...
  exchangeRates: {
    USD: 1, EUR: 0.92, GBP: 0.79, NGN: 1600, CAD: 1.37, AUD: 1.52,
    GHS: 15.5, ZAR: 18.2, KES: 129, UGX: 3700, TZS: 2550, RWF: 1370,
    XOF: 610, XAF: 610, INR: 83.5, AED: 3.6725, SAR: 3.75, JPY: 157,
    CNY: 7.25, BRL: 5.45, MXN: 18.2
  },
  ...
  /** Load current USD exchange rates through the backend. Cached locally for 12 hours. */
  async loadExchangeRates() {
    const cacheKey = 'tynaExchangeRatesV2';
    try {
      const cached = JSON.parse(localStorage.getItem(cacheKey) || 'null');
      if (cached && cached.savedAt && Date.now() - cached.savedAt < 43200000 && cached.rates) {
        this.exchangeRates = { ...this.exchangeRates, ...cached.rates, USD: 1 };
        return;
      }
      const base = (window.TYNA_CONFIG && window.TYNA_CONFIG.API_BASE_URL) || '';
      const response = await fetch(`${String(base).replace(/\/$/, '')}/api/payments/exchange-rates`);
      if (!response.ok) throw new Error('Exchange-rate service unavailable');
      const payload = await response.json();
      if (payload && payload.rates) {
        this.exchangeRates = { ...this.exchangeRates, ...payload.rates, USD: 1 };
        localStorage.setItem(cacheKey, JSON.stringify({ savedAt: Date.now(), rates: payload.rates }));
        this.updatePageCurrency();
      }
    } catch (error) {
      console.warn('Using built-in exchange-rate estimates:', error.message);
    }
  },
  ...
  formatCurrency(amount, currency = this.currentCurrency) {
    try {
      ...
      const item = this.getCurrencies()[currency] || { symbol: currency };
      return `${item.symbol || currency}${Number(amount || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}`;
    }
  },
   ...
   */
  getCurrencies() {
    return (window.TYNA_COUNTRY_CURRENCY_DATA && window.TYNA_COUNTRY_CURRENCY_DATA.currencies) || {
      USD: { name: 'US Dollar', symbol: '$' }, NGN: { name: 'Nigerian Naira', symbol: '₦' }
    };
  },

  getCountries() {
    return (window.TYNA_COUNTRY_CURRENCY_DATA && window.TYNA_COUNTRY_CURRENCY_DATA.countries) || [];
  },
   ...
   * Initialize i18n
   */
  applyCurrencyToElement(el) {
    const raw = el.getAttribute('data-price-usd') || el.getAttribute('data-usd-amount');
    const baseAmount = Number(raw);
    if (!Number.isFinite(baseAmount)) return;
    const usd = this.formatCurrency(baseAmount, 'USD');
    const converted = this.convertCurrency(baseAmount, 'USD', this.currentCurrency);
    const local = this.formatCurrency(converted, this.currentCurrency);
    const suffix = el.getAttribute('data-price-suffix') || '';
    if (el.matches('input, textarea')) {
      el.value = this.currentCurrency === 'USD' ? `${usd}${suffix}` : `${usd} / ${local} ${this.currentCurrency}${suffix}`;
    } else if (this.currentCurrency === 'USD') {
      el.textContent = `${usd} USD${suffix}`;
    } else {
      el.innerHTML = `<span class="tyna-price-usd">${usd} USD${suffix}</span><small class="tyna-price-local">${local} ${this.currentCurrency} equivalent</small>`;
    }
    el.setAttribute('data-active-currency', this.currentCurrency);
  },

  prepareStaticUsdPrices() {
    document.querySelectorAll('.price').forEach(el => {
      if (el.hasAttribute('data-price-usd') || el.hasAttribute('data-usd-amount')) return;
      const text = (el.textContent || '').trim();
      const match = text.match(/^\$([0-9][0-9,]*(?:\.[0-9]+)?)(.*)$/);
      if (!match) return;
      el.setAttribute('data-price-usd', match[1].replace(/,/g, ''));
      if (match[2]) el.setAttribute('data-price-suffix', match[2]);
    });
  },

  updatePageCurrency() {
    this.prepareStaticUsdPrices();
    document.querySelectorAll('[data-price-usd], [data-usd-amount]').forEach(el => this.applyCurrencyToElement(el));
    document.documentElement.setAttribute('data-currency', this.currentCurrency);
    document.querySelectorAll('[data-current-currency]').forEach(el => { el.textContent = this.currentCurrency; });
  },

  observeDynamicPrices() {
    if (this._currencyObserver || !document.body) return;
    this._currencyObserver = new MutationObserver(mutations => {
      for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
          if (!(node instanceof Element)) continue;
          if (node.matches('[data-price-usd], [data-usd-amount]')) this.applyCurrencyToElement(node);
          node.querySelectorAll?.('[data-price-usd], [data-usd-amount]').forEach(el => this.applyCurrencyToElement(el));
        }
      }
    });
    this._currencyObserver.observe(document.body, { childList: true, subtree: true });
  },
  ...
  init() {
    ...
    this.loadExchangeRates().then(() => this.updatePageCurrency()).catch(() => this.updatePageCurrency());
  }
};

window.TynaCurrency = {
  get code() { return i18n.currentCurrency; },
  convert(amount, from = 'USD', to = i18n.currentCurrency) { return i18n.convertCurrency(Number(amount), from, to); },
  format(amount, currency = i18n.currentCurrency) { return i18n.formatCurrency(Number(amount), currency); },
  fromUSD(amount) { return i18n.formatCurrency(i18n.convertCurrency(Number(amount), 'USD', i18n.currentCurrency), i18n.currentCurrency); },
  set(currency) { i18n.setCurrency(currency); }
};
